...
function tech_techer(&$c, $turns = 1)
{
    ...
    //$market_info = get_market_info();   //get the Public Market info
    //global $market;
    $techfloor = 500; //can lower this even more with ^2 on the weighting.
    $mil  = max(PublicMarket::price('mil') - $techfloor, rand(0, 300))^2; //because we are seeing that low demand techs teched more than whats reasonable now
    $med  = max(PublicMarket::price('med') - $techfloor, rand(0, 5))^2;
    $bus  = max(PublicMarket::price('bus') - $techfloor, rand(10, 400))^2;
    $res  = max(PublicMarket::price('res') - $techfloor, rand(10, 400))^2;
    $agri = max(PublicMarket::price('agri') - $techfloor, rand(10, 300))^2;
    $war  = max(PublicMarket::price('war') - $techfloor, rand(0, 10))^2;
    $ms   = max(PublicMarket::price('ms') - $techfloor, rand(0, 20))^2;
    $weap = max(PublicMarket::price('weap') - $techfloor, rand(0, 20))^2;
    $indy = max(PublicMarket::price('indy') - $techfloor, rand(5, 300))^2;
    $spy  = max(PublicMarket::price('spy') - $techfloor, rand(0, 10))^2;
    $sdi  = max(PublicMarket::price('sdi') - $techfloor, rand(2, 150))^2;
